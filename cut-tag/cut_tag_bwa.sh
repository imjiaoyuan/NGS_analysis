#!/bin/bash
#SBATCH --job-name=CUTnTag_bwa
#SBATCH -c 8
#SBATCH --mem=16G
#SBATCH -D /mnt/may1nov1/u5023/pal/cut-tag
#SBATCH -o /mnt/may1nov1/u5023/pal/cut-tag/logs/CUTnTag_bwa_%j.out
#SBATCH -e /mnt/may1nov1/u5023/pal/cut-tag/logs/CUTnTag_bwa_%j.err
...
BWA_INDEX="/mnt/may1nov1/u5023/pal/cut-tag/bwa_index/Palv.hap2.chr.genome"
...
if [[ -z "${CONDA_DEFAULT_ENV}" ]]; then
    echo "conda激活失败" >> ${LOG_DIR}/CUTnTag_bwa_${SLURM_JOB_ID}.log
    exit 1
fi

# Create BWA index if it doesn't exist
if [ ! -f "${BWA_INDEX}.bwt" ]; then
    bwa index -p ${BWA_INDEX} ${GENOME_FASTA} >> ${LOG_DIR}/CUTnTag_bwa_${SLURM_JOB_ID}.log 2>&1
fi

# Get all sample files (paired-end reads)
FASTQ_FILES_R1=($(ls ${RAW_DIR}/*_1.fq.gz ${RAW_DIR}/*_1_*.fq.gz 2>> ${LOG_DIR}/CUTnTag_bwa_${SLURM_JOB_ID}.log | sort -V))
FASTQ_FILES_R2=($(ls ${RAW_DIR}/*_2.fq.gz ${RAW_DIR}/*_2_*.fq.gz 2>> ${LOG_DIR}/CUTnTag_bwa_${SLURM_JOB_ID}.log | sort -V))
...
parallel --link -j 1 "  # Reduced to 1 job per node since each job uses 8 threads
    SAMPLE_NAME=\$(basename {1} _1.fq.gz | sed 's/_1_001//' | sed 's/_1//');
    SAMPLE_LOG=\"${LOG_DIR}/CUTnTag_bwa_\${SLURM_JOB_ID}_${SAMPLE_NAME}.log\";
